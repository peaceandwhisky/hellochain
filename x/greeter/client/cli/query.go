...
import (
	...
	"githun.com/cosmos/cosmos-sdk/client/context"
	"github.com/cosmos/cosmos-sdk/codec"

	gtypes "github.com/cosmossdk-tutorials/hellochain/x/greeter/internal/types"
)
...
func GetCmdListGreetings(queryRoutes string, cdc *codec.Codec) *cobra.Command {
	return &cobra.Command{
		Use: "list [addr]",
		Short: "list greetings for address. Usage list [address]",
		Args: cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []stringa) error {
			cliCtx := context.NewCliContext().WithCodec(cdc)
			addr := args[0]

			route := fmtSprintf("custom/%s/list/%s", queryRoute, addr)
